...
document.addEventListener('DOMContentLoaded', function () {
    initTabs();
    loadAccounts();
    loadGmailApiAccounts(); // Load Gmail API accounts
                ...
                if (instance.healthy) {
                    resultHtml += `
                        <small style="color: #00ff9d;">
                            ...
                            ✓ Relay-reported public IP: ${instance.relay_public_ip || instance.public_ip}<br>
...
function isEc2ConnectionRefusedError(message) {
    const msg = String(message || '').toLowerCase();
    return msg.includes('connection refused') || msg.includes('errno 111');
}

async function autoHealEc2Relay(instanceId, resultElementId) {
    try {
        showResult(resultElementId, '🔧 EC2 relay not reachable. Attempting auto-repair...', 'info');

        // First try a fast restart via SSM.
        await safeFetchJson('/api/ec2_management', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'restart_relay', instance_id: instanceId })
        });
        ...
        // Then force relay reinstall/update to ensure latest script is active.
        await safeFetchJson('/api/ec2_management', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'fix_relay', instance_id: instanceId })
        ...
        showResult(resultElementId, '✅ Auto-repair started. Retrying send...', 'info');
        await sleep(12000);
        return true;
    } catch (e) {
        showResult(resultElementId, `❌ Auto-repair failed: ${e.message}`, 'error');
        return false;
    }
}
...
function updateBulkStats() {
    ...
    } else if (method === 'gmail_api') {
        smtpCount = gmailApiAccounts.length;
    } else if (method === 'ec2') {
        smtpCount = ec2Instances.filter(i => i.state === 'running').length;
    } else if (method === 'ec2_gmail_api') {
        smtpCount = ec2Instances.filter(i => i.state === 'running').length;
    }
...
function contentRiskGuard(subject, html, attachment) {
    const issues = [];
    const lowerSubject = (subject || '').toLowerCase();
    const lowerHtml = (html || '').toLowerCase();
    const subjectHits = _SPAM_SUBJECT_WORDS.filter(w => lowerSubject.includes(w));

    if (subjectHits.length >= 2) {
        issues.push(`Subject contains high-risk keywords: ${subjectHits.slice(0, 3).join(', ')}`);
    }

    const linkCount = ((html || '').match(/https?:\/\//gi) || []).length;
    if (linkCount > 6) {
        issues.push(`Body has too many links (${linkCount}). Keep links under 6 during warmup.`);
    }

    if (attachment && (attachment.type === 'text/html' || /\.(html?|js|hta)$/i.test(attachment.name || ''))) {
        issues.push('HTML/script-like attachments are high-risk. Use PDF, PNG, or JPG instead.');
    }

    if (lowerHtml.includes('display:none') || lowerHtml.includes('visibility:hidden')) {
        issues.push('Hidden HTML/CSS detected. Hidden content is a spam signal.');
    }

    return issues;
}

function _extractSenderForGuard(method, config) {
    if (method === 'smtp') return (config.smtp_config && config.smtp_config.user) || '';
    if (method === 'ses') return (config.aws_config && config.aws_config.from_email) || '';
    if (method === 'ec2') return (config.smtp_config && config.smtp_config.user) || '';
    return '';
}

async function preflightDeliverabilityGuard(method, config, resultElementId) {
    const sender = _extractSenderForGuard(method, config);
    if (!sender || !sender.includes('@')) {
        showResult(resultElementId, '❌ Sender email missing. Configure SMTP/SES first.', 'error');
        return false;
    }

    const domain = sender.split('@')[1];
    showResult(resultElementId, `🔍 Pre-send auth check for ${domain}...`, 'info');

    const resp = await safeFetchJson('/api/deliverability', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'check_domain', domain, smtp_user: sender })
    });

    if (!resp.success) {
        showResult(resultElementId, `❌ Could not verify domain auth: ${resp.error || 'unknown error'}`, 'error');
        return false;
    }
    ...
    const mx = checks.mx || {};
    const badSpfPolicy = String(spf.record || '').includes('+all');

    const critical = [];
    if (spf.status === 'MISSING' || badSpfPolicy) critical.push('SPF');
    if (dmarc.status === 'MISSING') critical.push('DMARC');
    if (mx.status === 'MISSING') critical.push('MX');

    if (critical.length > 0) {
        showResult(resultElementId, `❌ Send blocked: critical auth issue (${critical.join(', ')}). Fix DNS in Deliverability Checker first.`, 'error');
        return false;
    }
...
async function sendSingleEmail() {
    ...
    if (method === 'smtp') {
        ...
        if (gmailApiAccounts.length === 0) {
            showResult('singleResult', 'Please add a Gmail API account first (Gmail API tab)', 'error');
            return;
        }
        config.gmail_config = gmailApiAccounts[0];
    } else if (method === 'ses') {
        if (sesAccounts.length === 0) {
        ...
        const scopeWarning = getEc2GmailScopeWarning(config.gmail_config);
        if (scopeWarning) {
            showResult('singleResult', scopeWarning, 'error');
            return;
        }
    } else if (method === 'ec2') {
        if (ec2Instances.length === 0) {
    ...
    // Optional advisory checks (non-blocking)
    const _singleSender = _extractSenderForGuard(method, config);
    if (_singleSender) {
        const _sentToday = getTodaySendCount(_singleSender);
        if (_sentToday > 200) {
            showResult('singleResult', `⚠️ Advisory: ${_singleSender} sent ${_sentToday} today. Consider pacing if issues occur.`, 'info');
        }
    }
    ...
    // Build inline HTML with embedded preview (centered, no "Attached:" notification)
    const _singleEmbedVisual = !!(document.getElementById('singleEmbedVisual') || { checked: true }).checked;
    const _singleAttachmentMode = document.getElementById('singleAttachmentMode')?.value || 'download';
    const singleInlineResult = buildInlineEmailHtml(html, attachment, { 
        embedVisual: _singleEmbedVisual,
        attachmentMode: _singleAttachmentMode
    });
    ...
    try {
        ...
        const _singlePayload = {
            to: to,
            subject: subject,
            html: singleInlineResult.html,
            method: method,
        ...
        let data;
        let _singleRetriedAfterHeal = false;
        for (let _attempt = 0; _attempt < 2; _attempt++) {
            try {
                data = await safeFetchJson('/api/send', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(_singlePayload)
                });
                ...
                const _canHeal = (method === 'ec2' || method === 'ec2_gmail_api')
                    && config.ec2_instance
                    && !_singleRetriedAfterHeal
                    && isEc2ConnectionRefusedError(data.error);

                if (_canHeal) {
                    _singleRetriedAfterHeal = await autoHealEc2Relay(config.ec2_instance.instance_id, 'singleResult');
                    if (_singleRetriedAfterHeal) {
                        continue;
                    }
                }
                break;
                ...
                const _canHeal = (method === 'ec2' || method === 'ec2_gmail_api')
                    && config.ec2_instance
                    && !_singleRetriedAfterHeal
                    && isEc2ConnectionRefusedError(_sendErr.message);
        ...
        if (data.success) {
            ...
            // If backend auto-refreshed the Gmail token, update localStorage
            if (data.new_access_token && config.gmail_config) {
                updateGmailAccessToken(config.gmail_config.user, data.new_access_token);
            }
...
async function sendBulkEmails() {
    ...
    if (method === 'smtp') {
        ...
        if (gmailApiAccounts.length === 0) {
            showResult('bulkResult', 'Please add at least one Gmail API account first (Gmail API tab)', 'error');
            return;
        }
    } else if (method === 'ses') {
        if (sesAccounts.length === 0) {
        ...
        const missingScopeAccounts = gmailApiAccounts.filter(acc => !accountHasSmtpScope(acc));
        if (missingScopeAccounts.length > 0) {
            showResult(
                'bulkResult',
                `⚠️ ${missingScopeAccounts.length} Gmail account(s) missing https://mail.google.com/ scope. Re-authenticate those account(s) in Gmail API tab before using EC2 + Gmail API SMTP-IP path.`,
                'error'
            );
            return;
        }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (_isRandName) {
            ...
            // Resolve from current SMTP or Gmail API account
            const _curSmtp = (method === 'smtp' || method === 'ec2') && smtpAccounts.length > 0
                ? smtpAccounts[rotateIdx % smtpAccounts.length]
                : null;
            const _curGmail = (method === 'gmail_api' || method === 'ec2_gmail_api') && gmailApiAccounts.length > 0
                ? gmailApiAccounts[rotateIdx % gmailApiAccounts.length]
                : null;
            const _cfgSenderName = _curSmtp ? (_curSmtp.sender_name || '') : (_curGmail ? (_curGmail.sender_name || '') : '');
            emailPayload.from_name = _sName
                || (_cfgSenderName && _cfgSenderName !== 'KINGMAILER' ? _cfgSenderName : '')
                || (_curSmtp ? (_curSmtp.user || '') : '')
                || (_curGmail ? (_curGmail.user || '') : '')
                || '';
        }
        ...
        if (method === 'smtp') {
            ...
            emailPayload.gmail_config = gmailApiAccounts[rotateIdx % gmailApiAccounts.length];
            // Rotate Gmail API account after each batch
            const batchSz = Math.max(1, parseInt((document.getElementById('batchSize') || {}).value) || 50);
            if ((i + 1) % batchSz === 0 && gmailApiAccounts.length > 1) {
                rotateIdx++;
                const batchInfoEl = document.getElementById('batchInfoDisplay');
                if (batchInfoEl) {
                    const acc = gmailApiAccounts[rotateIdx % gmailApiAccounts.length];
                    batchInfoEl.style.display = 'block';
                    batchInfoEl.textContent = `🔄 Gmail API rotated → ${acc.user} (account ${(rotateIdx % gmailApiAccounts.length) + 1}/${gmailApiAccounts.length}, after ${i + 1} emails)`;
                }
            }
        } else if (method === 'ses') {
        ...
        const _bulkAttachmentMode = document.getElementById('bulkAttachmentMode')?.value || 'download';
        const bulkInlineResult = buildInlineEmailHtml(emailPayload.html, attachment, { 
            embedVisual: _bulkEmbedVisual,
            attachmentMode: _bulkAttachmentMode
        });
        ...
        try {
            let result;
            let _bulkRetried = false;

            for (let _attempt = 0; _attempt < 2; _attempt++) {
                try {
                    result = await safeFetchJson('/api/send', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    const _canHealBulk = (method === 'ec2' || method === 'ec2_gmail_api')
                        && emailPayload.ec2_instance
                        && !bulkEc2AutoHealed
                        && !_bulkRetried
                        && isEc2ConnectionRefusedError(result.error);

                    if (_canHealBulk) {
                        bulkEc2AutoHealed = await autoHealEc2Relay(emailPayload.ec2_instance.instance_id, 'bulkResult');
                        _bulkRetried = bulkEc2AutoHealed;
                        if (bulkEc2AutoHealed) {
                            continue;
                        }
                    }
                    break;
                    ...
                    const _canHealBulk = (method === 'ec2' || method === 'ec2_gmail_api')
                        ...
                        && !bulkEc2AutoHealed
                        && !_bulkRetried
                        && isEc2ConnectionRefusedError(_errSend.message);
            ...
            if (result.success) {
                ...
                const _bTrackUser = emailPayload.smtp_config ? emailPayload.smtp_config.user 
                    : (emailPayload.gmail_config ? emailPayload.gmail_config.user 
                    : (emailPayload.aws_config ? emailPayload.aws_config.from_email : ''));
                if (_bTrackUser) recordSend(_bTrackUser);
                
                // If backend auto-refreshed the Gmail token, update localStorage
                if (result.new_access_token && emailPayload.gmail_config) {
                    updateGmailAccessToken(emailPayload.gmail_config.user, result.new_access_token);
                }
...
function getHeaderOpts(context) {
    return {
        ...
        reply_to:         !!(document.getElementById(context + 'HeaderReplyTo') || {}).checked,
    };
}
...
function generateAttachName(format) {
    ...
    // Alphanumeric chars — pure numeric filenames are a top spam signal.
    // Real company docs use reference codes like "A3K8P-B7X2R4" not "73291-847362".
    // Excludes I/O/0/1 to avoid visual confusion.
    const chars = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
    const rndSeg = (n) => Array.from({length: n}, () => chars[Math.floor(Math.random() * chars.length)]).join('');
    ...
    return format.split('-').map(seg => rndSeg(parseInt(seg) || 5)).join('-');
}
...
// Build inline HTML email with centered preview (no attachment notification, centered layout)
function buildInlineEmailHtml(baseHtml, attachment, opts) {
    opts = opts || {};
    const embedVisual = opts.embedVisual !== false; // Default true
    const attachmentMode = opts.attachmentMode || 'download'; // 'download' or 'inline'
...
function hasMailGoogleScope(scopes) {
    const arr = Array.isArray(scopes) ? scopes : parseScopeString(scopes);
    return arr.includes('https://mail.google.com/');
}

function accountHasSmtpScope(account) {
    if (!account) return false;
    if (account.has_smtp_scope === true) return true;
    return hasMailGoogleScope(account.granted_scopes || '');
}
...
async function fetchTokenScopes(accessToken) {
    try {
        ...
        if (!resp.ok) return { scopes: [], hasSmtpScope: false };
        const data = await resp.json();
        const scopes = parseScopeString(data.scope || '');
        return { scopes, hasSmtpScope: hasMailGoogleScope(scopes) };
    } catch (_) {
        return { scopes: [], hasSmtpScope: false };
    }
}

function getEc2GmailScopeWarning(account) {
    if (accountHasSmtpScope(account)) return '';
    return '⚠️ This token is missing https://mail.google.com/ scope. Re-authenticate in Gmail API tab before using EC2 + Gmail API (for EC2 SMTP-IP path).';
}

// Load credentials from uploaded JSON file (from Google Cloud Console)
function loadGmailApiCredentialsFromJson(event) {
    const file = event.target.files[0];
    if (!file) return;
    ...
    reader.onload = function(e) {
        try {
            ...
            // Google Cloud credentials JSON can have different structures
            let clientId = '';
            let clientSecret = '';
            let redirectUris = [];
            let credentialType = '';
            ...
            if (json.installed) {
                credentialType = 'installed';
                clientId = json.installed.client_id || '';
                clientSecret = json.installed.client_secret || '';
                redirectUris = json.installed.redirect_uris || [];
                authUri = json.installed.auth_uri || authUri;
                tokenUri = json.installed.token_uri || tokenUri;
            } else if (json.web) {
                credentialType = 'web';
                clientId = json.web.client_id || '';
                clientSecret = json.web.client_secret || '';
                redirectUris = json.web.redirect_uris || [];
                authUri = json.web.auth_uri || authUri;
                tokenUri = json.web.token_uri || tokenUri;
            } else {
                clientId = json.client_id || '';
                clientSecret = json.client_secret || '';
                redirectUris = json.redirect_uris || [];
                credentialType = 'unknown';
                authUri = json.auth_uri || authUri;
                tokenUri = json.token_uri || tokenUri;
            }
            
            if (!clientId || !clientSecret) {
                showResult('gmailApiResult', '❌ Invalid credentials file. Make sure it contains client_id and client_secret.', 'error');
                return;
            }
            
            // Store credentials globally for OAuth
            window.gmailOAuthConfig = {
                client_id: clientId,
            ...
            // Fill credentials in readonly fields
            document.getElementById('gmailApiClientId').value = clientId;
            document.getElementById('gmailApiClientSecret').value = clientSecret;
            
            // Enable the "Authenticate" button
            const authBtn = document.getElementById('authenticateGmailBtn');
            if (authBtn) {
                authBtn.disabled = false;
                authBtn.style.opacity = '1';
            }
            
            // Show success - ready to authenticate
            document.getElementById('authBtnHint').innerHTML = 
                `✅ <strong style="color:#00ff9d;">Credentials loaded successfully!</strong><br>` +
                `<small style="color:#888;">Click "Authenticate with Google" below to sign in</small>`;
            
            showResult('gmailApiResult', 
                `✅ <strong>Credentials loaded!</strong><br><br>` +
                `<strong>Type:</strong> ${credentialType === 'installed' ? 'Desktop App' : 'Web Application'}<br>` +
                `<strong>Client ID:</strong> ${clientId.substring(0, 40)}...<br><br>` +
                `Click <strong>"Authenticate with Google"</strong> to sign in!`, 
                'success'
            );
            ...
            showResult('gmailApiResult', `❌ Failed to parse JSON file: ${err.message}`, 'error');
        }
    };
    
    reader.onerror = function() {
        showResult('gmailApiResult', '❌ Failed to read file', 'error');
    };
    
    reader.readAsText(file);
}

// Authenticate with Google OAuth2 - works with both Desktop and Web credentials
function authenticateWithGoogle() {
    if (!window.gmailOAuthConfig) {
        showResult('gmailApiResult', '❌ Please upload credentials.json first', 'error');
        return;
    }
    
    const { client_id, credential_type } = window.gmailOAuthConfig;

    // Google deprecated OOB. For installed credentials we must use a loopback URI
    // from the uploaded JSON (usually http://localhost or http://127.0.0.1).
    const redirectUri = credential_type === 'installed'
        ? resolveInstalledRedirectUri(window.gmailOAuthConfig)
        : `${window.location.origin}/oauth-callback.html`;

    if (credential_type === 'installed' && !redirectUri) {
        showResult(
            'gmailApiResult',
            '❌ This desktop credentials file has no localhost redirect URI. Re-download credentials.json from Google Cloud and try again.',
            'error'
        );
        return;
    }
    
    // Include openid scope for proper OAuth2 flow
    // Include mail.google.com for SMTP XOAUTH2 (EC2 relay SMTP mode) and gmail.send for API mode.
    const scope = 'openid email https://mail.google.com/ https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/userinfo.email';
    
    const authBaseUri = window.gmailOAuthConfig.auth_uri || 'https://accounts.google.com/o/oauth2/v2/auth';
    const authUrl = authBaseUri + '?' + new URLSearchParams({
        client_id: client_id,
    ...
    if (credential_type === 'installed') {
        // Desktop app flow - open window, user copies code manually
        const width = 600;
        const height = 700;
        ...
        showResult('gmailApiResult', 
            `🔐 <strong>Google sign-in window opened!</strong><br><br>` +
            `<strong>Steps:</strong><br>` +
            `1. Sign in with your Google account<br>` +
            `2. Click "Allow" to authorize<br>` +
            `3. Browser will redirect to localhost and may show a connection error (this is normal)<br>` +
            `4. Copy the FULL URL from that browser address bar and paste below (or paste only the code):<br><br>` +
            `<div style="margin:15px 0;">` +
            `<input type="text" id="manualAuthCode" placeholder="Paste full localhost URL OR authorization code" style="width:100%; padding:10px; background:#1a1a1a; border:1px solid #00ff9d; color:#00ff9d; border-radius:6px; font-family:monospace; font-size:14px;"><br>` +
            `<button onclick="exchangeManualCode()" style="margin-top:10px; padding:10px 20px; background:#00ff9d; color:#000; border:none; border-radius:6px; font-weight:bold; cursor:pointer;">Submit Code</button>` +
            `</div>`, 
            'info'
        );
    } else {
        // Web app flow - automatic popup callback
        const width = 500;
        const height = 600;
        const left = (screen.width / 2) - (width / 2);
        const top = (screen.height / 2) - (height / 2);
        ...
        if (!popup) {
            showResult('gmailApiResult', '❌ Popup blocked! Please allow popups for this site.', 'error');
            return;
        }
        
        showResult('gmailApiResult', '🔄 Waiting for Google sign-in...', 'info');
        
        // Listen for the authorization code from the popup
        window.gmailAuthCallback = async function(code) {
            popup.close();
            await processAuthCode(code, redirectUri);
        };
    }
}

function resolveInstalledRedirectUri(config) {
    const redirectUris = Array.isArray(config?.redirect_uris) ? config.redirect_uris : [];
    if (!redirectUris.length) {
        return '';
    }

    const preferred = redirectUris.find(uri =>
        typeof uri === 'string' &&
...
function extractAuthCode(rawInput) {
    ...
    // If user pasted a URL, extract ?code=... from it.
    ...
    // If user pasted just the code, use as-is.
    return input;
}

// Exchange manually pasted authorization code
window.exchangeManualCode = async function() {
    const rawInput = document.getElementById('manualAuthCode').value.trim();
    const code = extractAuthCode(rawInput);
    if (!code) {
        showResult('gmailApiResult', '❌ Please paste the full localhost URL or authorization code', 'error');
        return;
    }
    
    const redirectUri = resolveInstalledRedirectUri(window.gmailOAuthConfig);
    if (!redirectUri) {
        showResult('gmailApiResult', '❌ Missing installed-app redirect URI in credentials.json', 'error');
        return;
    }
...
async function processAuthCode(code, redirectUri) {
    showResult('gmailApiResult', '🔄 Exchanging authorization code for tokens...', 'info');
    
    try {
        // Exchange code for tokens via our API
        const response = await fetch('/api/exchange_token', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                code: code,
                client_id: window.gmailOAuthConfig.client_id,
                client_secret: window.gmailOAuthConfig.client_secret,
                redirect_uri: redirectUri,
                token_uri: window.gmailOAuthConfig.token_uri || 'https://oauth2.googleapis.com/token'
            })
        });
        ...
        if (!data.success) {
            showResult('gmailApiResult', `❌ Token exchange failed: ${data.error}`, 'error');
            return;
        }
        
        // Got tokens! Auto-save account
        const senderName = document.getElementById('gmailApiSenderName').value.trim();
        const tokenScopeInfo = await fetchTokenScopes(data.access_token);
        ...
        const account = {
            ...
            sender_name: senderName || '',
            provider: 'gmail-api',
            granted_scopes: tokenScopeInfo.scopes,
            has_smtp_scope: tokenScopeInfo.hasSmtpScope,
            token_uri: window.gmailOAuthConfig.token_uri || 'https://oauth2.googleapis.com/token'
        };
        
        // Check if account already exists
        const existingIndex = gmailApiAccounts.findIndex(acc => acc.user === data.user_email);
        if (existingIndex >= 0) {
            gmailApiAccounts[existingIndex] = account;
            showResult('gmailApiResult', 
                `✅ <strong>Account updated successfully!</strong><br><br>` +
                `<strong>Email:</strong> ${data.user_email}<br>` +
                `<strong>Status:</strong> Ready to send emails<br>` +
                `${tokenScopeInfo.hasSmtpScope ? '<small style="color:#00ff9d;">SMTP scope: https://mail.google.com/ ✓</small>' : '<small style="color:#fbbf24;">SMTP scope missing. Re-auth required for EC2 SMTP-IP path.</small>'}<br><br>` +
                `You can now send emails! Tokens will auto-refresh when needed. 🎉`, 
                'success'
            );
        } else {
            gmailApiAccounts.push(account);
            showResult('gmailApiResult', 
                `✅ <strong>Account added successfully!</strong><br><br>` +
                `<strong>Email:</strong> ${data.user_email}<br>` +
                `<strong>Status:</strong> Ready to send emails<br>` +
                `${tokenScopeInfo.hasSmtpScope ? '<small style="color:#00ff9d;">SMTP scope: https://mail.google.com/ ✓</small>' : '<small style="color:#fbbf24;">SMTP scope missing. Re-auth required for EC2 SMTP-IP path.</small>'}<br><br>` +
                `You can now send emails! Tokens will auto-refresh when needed. 🎉`, 
        ...
        saveGmailApiAccounts();
        updateGmailApiAccountsList();
        
        // Clear sender name field
        document.getElementById('gmailApiSenderName').value = '';
        
    } catch (error) {
        showResult('gmailApiResult', `❌ Authentication failed: ${error.message}`, 'error');
    }
}

// Message listener for OAuth callback
window.addEventListener('message', function(event) {
    if (event.data && event.data.type === 'GMAIL_OAUTH_CODE') {
        if (window.gmailAuthCallback) {
            window.gmailAuthCallback(event.data.code);
        }
    }
}, false);
...
function copyToClipboard(text, label) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(() => {
            showResult('gmailApiResult', `✅ ${label} copied to clipboard!`, 'success');
        }).catch(() => {
...
function fallbackCopyToClipboard(text, label) {
    ...
    try {
        ...
        showResult('gmailApiResult', `❌ Failed to copy. Please copy manually.`, 'error');
    }
...
function updateGmailAccessToken(gmailUser, newAccessToken) {
    // Find and update the Gmail API account with the new access token (auto-refreshed)
    const account = gmailApiAccounts.find(acc => acc.user === gmailUser);
    if (account) {
        account.access_token = newAccessToken;
        saveGmailApiAccounts();
        console.log(`[Auto-Refresh] Updated access token for ${gmailUser}`);
        return true;
    }
    return false;
}

async function addGmailApiAccount() {
    const user = (document.getElementById('gmailApiUser') || {}).value.trim();
    const accessToken = (document.getElementById('gmailApiAccessToken') || {}).value.trim();
    const refreshToken = (document.getElementById('gmailApiRefreshToken') || {}).value.trim();
    const clientId = (document.getElementById('gmailApiClientId') || {}).value.trim();
    const clientSecret = (document.getElementById('gmailApiClientSecret') || {}).value.trim();
    const senderName = (document.getElementById('gmailApiSenderName') || {}).value.trim();

    if (!user || !accessToken || !clientId || !clientSecret) {
        showResult('gmailApiResult', '⚠️ Please fill in: Gmail address, access token, client ID, and client secret', 'error');
        return;
    }

    const tokenScopeInfo = await fetchTokenScopes(accessToken);
    ...
    // Clear form
    document.getElementById('gmailApiUser').value = '';
    document.getElementById('gmailApiAccessToken').value = '';
    document.getElementById('gmailApiRefreshToken').value = '';
    document.getElementById('gmailApiClientId').value = '';
    document.getElementById('gmailApiClientSecret').value = '';
    document.getElementById('gmailApiSenderName').value = '';

    const scopeMsg = tokenScopeInfo.hasSmtpScope
        ? 'SMTP scope detected (https://mail.google.com/) ✓'
        : 'SMTP scope missing. Re-authenticate to use EC2 + Gmail API with SMTP-IP path.';
    showResult('gmailApiResult', `✅ Gmail API account added: ${user}<br><small>${scopeMsg}</small>`, 'success');
}
...
async function testGmailApiConnection() {
    ...
    if (!user || !accessToken || !clientId) {
        showResult('gmailApiResult', '⚠️ Fill in Gmail address, access token, and client ID to test', 'error');
        return;
    }

    showResult('gmailApiResult', '🔄 Testing Gmail API connection...', 'info');

    try {
        // Make a simple Gmail API request to verify credentials
        ...
        if (response.ok) {
            const data = await response.json();
            showResult('gmailApiResult', `✅ Gmail API connected successfully!\nEmail: ${data.emailAddress}\nMessages: ${data.messagesTotal || 'N/A'}`, 'success');
        } else if (response.status === 401) {
            showResult('gmailApiResult', '❌ Access token expired or invalid. Get a fresh token.', 'error');
        } else {
            const errorText = await response.text();
            showResult('gmailApiResult', `❌ Gmail API test failed: ${response.status} ${errorText}`, 'error');
        }
    } catch (err) {
        showResult('gmailApiResult', `❌ Gmail API test failed: ${err.message}`, 'error');
    }
}

function updateGmailApiAccountsList() {
    const listEl = document.getElementById('gmailApiAccountsList');
    if (!listEl) return;

    if (gmailApiAccounts.length === 0) {
        listEl.innerHTML = '<p style="color:#888;">No Gmail API accounts configured.</p>';
        return;
    }

    const needsReauth = gmailApiAccounts.filter(acc => !accountHasSmtpScope(acc));
    const summaryBanner = needsReauth.length > 0
        ? `
        <div style="background:rgba(248,113,113,0.10); border:1px solid #f87171; border-radius:8px; padding:12px 14px; margin-bottom:14px;">
            <strong style="color:#f87171;">Needs Re-Auth</strong><br>
            <small style="color:#fca5a5;">
                ${needsReauth.length} Gmail account(s) are missing https://mail.google.com/ scope.
                Re-authenticate them before using EC2 + Gmail API if you want Gmail SMTP XOAUTH2 from the EC2 relay.
            </small>
        </div>`
        : `
        <div style="background:rgba(0,255,157,0.08); border:1px solid #00ff9d; border-radius:8px; padding:12px 14px; margin-bottom:14px;">
            <strong style="color:#00ff9d;">All Gmail Accounts Ready</strong><br>
            <small style="color:#9ae6b4;">All saved Gmail accounts include https://mail.google.com/ scope.</small>
        </div>`;

    listEl.innerHTML = summaryBanner + gmailApiAccounts.map((acc, idx) => `
        <div class="account-item">
            <div>
                <strong>${acc.user}</strong>
                ${acc.sender_name ? `<br><small style="color:#888;">Name: ${acc.sender_name}</small>` : ''}
                <br><small style="color:#888;">Client ID: ${acc.client_id.substring(0, 20)}...</small>
                ${acc.refresh_token ? '<br><small style="color:#00ff9d;">Has refresh token ✓</small>' : '<br><small style="color:#fbbf24;">No refresh token - tokens will expire</small>'}
                ${accountHasSmtpScope(acc)
                    ? '<br><small style="color:#00ff9d;">Scope: https://mail.google.com/ ✓</small>'
                    : '<br><small style="color:#fbbf24;">Scope missing: https://mail.google.com/ (re-auth needed for EC2 + Gmail API SMTP-IP path)</small>'}
            </div>
            <button class="btn-delete" onclick="deleteGmailApiAccount(${idx})">Delete</button>
        </div>
    `).join('');
}

function deleteGmailApiAccount(index) {
    if (!confirm('Delete this Gmail API account?')) return;
    gmailApiAccounts.splice(index, 1);
    saveGmailApiAccounts();
    updateGmailApiAccountsList();
}
...
/**
 * Update fast mode status indicator based on delay settings
 */
function updateFastModeStatus() {
    const minDelayInput = document.getElementById('bulkMinDelay');
    const maxDelayInput = document.getElementById('bulkMaxDelay');
    const statusDiv = document.getElementById('fastModeStatus');
    
    if (!minDelayInput || !maxDelayInput || !statusDiv) return;
    ...
    if (maxDelay <= 1000 && minDelay <= 1000) {
        statusText = '🚀 FAST MODE ACTIVE - Minimal delays for rapid sending';
        statusColor = '#fff';
        bgColor = '#10b981';
    } else if (maxDelay <= 5000) {
        statusText = '⚡ SPEED MODE - Low delays for faster sending';
        statusColor = '#fff';
        bgColor = '#f59e0b';
    } else {
        statusText = '🐌 SAFE MODE - Conservative delays for maximum inbox rate';
        statusColor = '#fff';
        bgColor = '#3b82f6';
    }
    ...
    // Log fast mode detection for console visibility
    if (maxDelay <= 1000) {
        console.log('🚀 FAST MODE DETECTED: Max delay = ' + maxDelay + 'ms, using minimal delays for rapid sending');
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Give the DOM a moment to render before setting up fast mode status
    setTimeout(updateFastModeStatus, 100);
});
