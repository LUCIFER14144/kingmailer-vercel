    ...
    try:
        ...
        if os.path.exists(accounts_file):
            with open(accounts_file, 'r') as f:
                data = json.load(f)
                print(f"[ACCOUNT-STATS] SUCCESS Loaded saved accounts: SMTP: {len(data.get('smtp_accounts', []))}, SES: {len(data.get('ses_accounts', []))}, Gmail API: {len(data.get('gmail_api_accounts', []))}")
                return data
    except Exception as e:
        print(f"[ACCOUNT-STATS] WARNING Error loading saved accounts: {e}")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if data.get('success') and data.get('accounts'):
                ...
                print(f"[ACCOUNT-STATS] SUCCESS Loaded accounts via HTTP: SMTP: {len(accounts.get('smtp_accounts', []))}, SES: {len(accounts.get('ses_accounts', []))}, Gmail API: {len(accounts.get('gmail_api_accounts', []))}")
                return accounts
    except Exception as e:
        print(f"[ACCOUNT-STATS] WARNING HTTP fallback failed: {e}")
    
    print("[ACCOUNT-STATS] WARNING No saved accounts found - using empty structure")
    return {
        'smtp_accounts': [],
        'ses_accounts': [],
        'gmail_api_accounts': [],
...
def merge_accounts_with_stats():
    ...
    for account in saved_accounts.get('smtp_accounts', []):
        ...
        print(f"[ACCOUNT-STATS] SUCCESS Processing real SMTP account: {account_id}")
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            print(f"[ACCOUNT-STATS] SUCCESS Returned {total_accounts} accounts ({active_accounts} active, {deactivated_accounts} deactivated)")
    ...
    def do_POST(self):
        ...
        try:
            ...
            if action == 'sync_accounts':
                ...
                print("[ACCOUNT-STATS] SUCCESS Accounts synchronized via POST request")
                ...
                if account_type in tracking_stats and account_id in tracking_stats[account_type]:
                    ...
                    print(f"[ACCOUNT-STATS] SUCCESS Reactivated {account_type} account: {account_id}")
                else:
                    response_data = {
                        "success": True,
            ...
            print(f"[ACCOUNT-STATS] ERROR POST Error: {str(e)}")
            self.send_response(500)
            self.send_header('Content-Type', 'application/json')
            self.send_header('Access-Control-Allow-Origin', '*')
            self.end_headers()
