...
_RELAY_SCRIPT = (
    '#!/usr/bin/env python3\n'
    '# KINGMAILER Relay v9.1 — JetMailer Style (Authenticated SMTP via EC2 IP)\n'
    ...
    '   self.wfile.write(json.dumps({"status":"healthy","service":"KINGMAILER Relay v9.1","timestamp":datetime.now().isoformat(),"port_587":_port_status(587),"port_465":_port_status(465),"mode":"authenticated-smtp-via-ec2-ip","note":"JetMailer style - no port 25 needed"}).encode())\n'
    ...
    ' logging.info("KINGMAILER Relay v9.1 started on port 3000 (JetMailer style)")\n'
    ' srv2.serve_forever()\n'
)
